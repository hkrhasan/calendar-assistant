...
class CalendarAssistant:
  ...
  def parse_time(self, time_str: str, reference: datetime = None) -> dict:
    ...
    # Handle relative day references first
    base_date = reference
    if "tomorrow" in time_str:
      base_date = reference + timedelta(days=1)
    ...
    if time_str == "today" or not time_str:
    ...
    try:
      ...
      if " to " in time_str or "-" in time_str:
        ...
        # Parse start time relative to base_date
        start_dt = dateutil.parser.parse(start_part.strip(), fuzzy=True, default=base_date)
        if not start_dt.tzinfo:
          start_dt = self.user_timezone.localize(start_dt)
        else:
          start_dt = start_dt.astimezone(self.user_timezone)
        
        # Parse end time relative to start time
        end_dt = dateutil.parser.parse(end_part.strip(), fuzzy=True, default=start_dt)
        if not end_dt.tzinfo:
          end_dt = self.user_timezone.localize(end_dt)
        else:
          end_dt = end_dt.astimezone(self.user_timezone)
        
        return {"start": start_dt, "end": end_dt}
        
      # Parse single time point relative to base_date
      dt = dateutil.parser.parse(time_str, fuzzy=True, default=base_date)
      if not dt.tzinfo:
